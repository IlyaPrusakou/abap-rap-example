CLASS zpru_cl_u_po_virt_elem DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.
...
CLASS zpru_cl_u_po_virt_elem IMPLEMENTATION.
    ...
    LOOP AT it_requested_calc_elements ASSIGNING FIELD-SYMBOL(<lv_virtual_field_name>).
      LOOP AT ct_calculated_data ASSIGNING FIELD-SYMBOL(<ls_calculation_structure>).
        ...
        CASE <lv_original_field_value>.
          WHEN `M`.
            <lv_virtual_field_value> = `GREENFIELD`.
          WHEN `U`.
            <lv_virtual_field_value> = `BROWNFIELD`.
          WHEN OTHERS.
            <lv_virtual_field_value> = `UNSPECIFIED`.
        ENDCASE.
      ENDLOOP.
    ENDLOOP.
  ENDMETHOD.
